...
impl<B: Backend> BarretenbergApi<B> {
    ...
    /// Execute AvmProve command
    pub fn avm_prove(&mut self, inputs: &[u8]) -> Result<AvmProveResponse> {
        let cmd = Command::AvmProve(AvmProve::new(inputs.to_vec()));
        match self.execute(cmd)? {
            Response::AvmProveResponse(resp) => Ok(resp),
            Response::ErrorResponse(err) => Err(BarretenbergError::Backend(
                err.message
            )),
            _ => Err(BarretenbergError::InvalidResponse(
                "Expected AvmProveResponse".to_string()
            )),
        }
    }

    /// Execute AvmVerify command
    pub fn avm_verify(&mut self, proof: Vec<Vec<u8>>, public_inputs: &[u8]) -> Result<AvmVerifyResponse> {
        let cmd = Command::AvmVerify(AvmVerify::new(proof, public_inputs.to_vec()));
        match self.execute(cmd)? {
            Response::AvmVerifyResponse(resp) => Ok(resp),
            Response::ErrorResponse(err) => Err(BarretenbergError::Backend(
                err.message
            )),
            _ => Err(BarretenbergError::InvalidResponse(
                "Expected AvmVerifyResponse".to_string()
            )),
        }
    }

    /// Execute AvmCheckCircuit command
    pub fn avm_check_circuit(&mut self, inputs: &[u8]) -> Result<AvmCheckCircuitResponse> {
        let cmd = Command::AvmCheckCircuit(AvmCheckCircuit::new(inputs.to_vec()));
        match self.execute(cmd)? {
            Response::AvmCheckCircuitResponse(resp) => Ok(resp),
            Response::ErrorResponse(err) => Err(BarretenbergError::Backend(
                err.message
            )),
            _ => Err(BarretenbergError::InvalidResponse(
                "Expected AvmCheckCircuitResponse".to_string()
            )),
        }
    }
    ...
    pub fn chonk_compute_vk(&mut self, circuit: CircuitInputNoVK, use_zk_flavor: bool) -> Result<ChonkComputeVkResponse> {
        let cmd = Command::ChonkComputeVk(ChonkComputeVk::new(circuit, use_zk_flavor));
        match self.execute(cmd)? {
    ...
    /// Execute ChonkVerifyFromFields command
    pub fn chonk_verify_from_fields(&mut self, proof: Vec<Vec<u8>>, vk: &[u8]) -> Result<ChonkVerifyFromFieldsResponse> {
        let cmd = Command::ChonkVerifyFromFields(ChonkVerifyFromFields::new(proof, vk.to_vec()));
        match self.execute(cmd)? {
            Response::ChonkVerifyFromFieldsResponse(resp) => Ok(resp),
            Response::ErrorResponse(err) => Err(BarretenbergError::Backend(
                err.message
            )),
            _ => Err(BarretenbergError::InvalidResponse(
                "Expected ChonkVerifyFromFieldsResponse".to_string()
            )),
        }
    }
    ...
    pub fn chonk_check_precomputed_vk(&mut self, circuit: CircuitInput, use_zk_flavor: bool) -> Result<ChonkCheckPrecomputedVkResponse> {
        let cmd = Command::ChonkCheckPrecomputedVk(ChonkCheckPrecomputedVk::new(circuit, use_zk_flavor));
        match self.execute(cmd)? {
    ...
    pub fn schnorr_construct_signature(&mut self, message_field: &[u8], private_key: &[u8]) -> Result<SchnorrConstructSignatureResponse> {
        let cmd = Command::SchnorrConstructSignature(SchnorrConstructSignature::new(message_field.to_vec(), private_key.to_vec()));
        match self.execute(cmd)? {
    ...
    pub fn schnorr_verify_signature(&mut self, message_field: &[u8], public_key: GrumpkinPoint, s: &[u8], e: &[u8]) -> Result<SchnorrVerifySignatureResponse> {
        let cmd = Command::SchnorrVerifySignature(SchnorrVerifySignature::new(message_field.to_vec(), public_key, s.to_vec(), e.to_vec()));
        match self.execute(cmd)? {
    ...
    /// Execute ChonkBatchVerifierStart command
    ...
    /// Execute ChonkBatchVerifierQueue command
    ...
    /// Execute ChonkBatchVerifierStop command
